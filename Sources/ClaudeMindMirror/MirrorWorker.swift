...
public actor MirrorWorker {
    ...
    private func publishMemory(client: PostgresClient, memoryID: UUID) async throws {
        ...
        // Serialize the real continuous-coordinate metadata into the JSONB column
        // (was hardcoded "{}"); empty/absent → "{}".
        let metadataJSON: String = {
            guard let md = m.metadata, !md.isEmpty,
                  let data = try? JSONSerialization.data(withJSONObject: md, options: [.sortedKeys]),
